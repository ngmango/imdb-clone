...
// req.params, req.body, req.headers, req.query
...
app.get('/movie/:movieId', (req, res) => {
    ...
    for (let i=0; i<movieList.length; i++){
      if(id === movieList[i].id){
        mov = {movie: movieList[i]}
      }
    }
    res.render('movie.ejs', mov)
  
})

app.get('/search', (req, res) =>{
  let search = (req.query.movieTitle),  
    movy = getmovies().filter(movieList => movieList.title.toLowerCase().includes(search.toLowerCase()))

  res.render('search.ejs', {movy});
})
...
// req.query/search?movieTitle = " placename " & genre = "sci fi"
// {movieTitle: "placename",
// genre: 'sci fi'}